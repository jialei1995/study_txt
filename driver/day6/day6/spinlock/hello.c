...
spinlock_t  mylock;//定义自旋锁变量 
int deviceused = 0;// 1 ------ 设备被使用  0 ------- 设备没有被使用
int hello_open(struct inode *node, struct file *f)
{

	spin_lock(&mylock);//获取锁
	if(deviceused == 1)//对共享资源deviceused 判断 
	{
		spin_unlock(&mylock);//不能使用设备， 释放锁， return -1
		return -1;
	}
	else
	{
		deviceused =1;// 可以使用， 变量置 1， 表示设备被使用
	}
...
{
	spin_lock(&mylock);//关闭设备文件时， 变量置0， 表示设备可以重新使用
	deviceused = 0;
	spin_unlock(&mylock);//释放锁
