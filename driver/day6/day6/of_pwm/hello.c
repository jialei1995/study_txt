...
void *gpiocon, *tcfg0, *tcfg1, *tcon, *tcount, *tcmp;
...
void  pwm_init(void)
{
	...
	//GPD0CON = GPD0CON & ~0xf  | 2;
		tmp = ioread32(gpiocon);
		tmp = (tmp & ~0xf) | 2;
		iowrite32(tmp, gpiocon);
...
#endif
	gpiocon = ioremap(pres[0].start, 4);
	tcfg0 = ioremap(pres[1].start, 4);
	tcfg1 = ioremap(pres[2].start,4);
	tcon = ioremap(pres[3].start,4);
	tcount = ioremap(pres[4].start,4);
	tcmp = ioremap(pres[5].start,4);
